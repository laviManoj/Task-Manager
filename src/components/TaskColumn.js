...
const TaskColumn = ({ title, tasks, onDragStart, onDragOver, onDrop, onDelete, onEdit, onView }) => {
  return (
    <div 
      className="bg-white rounded-lg shadow-lg w-full md:w-[33%] justify-between flex-shrink-0"
      onDragOver={onDragOver}
      onDrop={(e) => onDrop(e, title)}
    >
      <h2 className="text-xl font-semibold text-white bg-blue-500 m-4 p-4 rounded-sm">{title}</h2>
      <div className="p-4">
        {tasks.map((task) => (
          <div 
            key={task.id} 
